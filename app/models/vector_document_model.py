...
class VectorDocument(Base):
    ...
    # Every exercise doc belongs to a specific gym
    gym_id = Column(UUID(as_uuid=True), ForeignKey("gyms.id", ondelete="CASCADE"), nullable=False)

    doc_type = Column(String(50), nullable=False)        # "exercise" | "nutrition" | etc.
    source_id = Column(String(100), nullable=True)       # exercise_id from JSONL e.g. "chest_001"
    content = Column(Text, nullable=False)               # rag_text used for embedding
    metadata_ = Column("metadata", JSON, nullable=True)  # full exercise payload as JSON
